...
Address::ptr Address::Create(const sockaddr* addr, socklen_t uint16_t) {
    if (!addr) {
        return nullptr;
    }
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port)
{
    ...
    addr->m_addr.sin_port = onBigEndian(port);
...
IPv4Address::IPv4Address(uint32_t address, uint16_t port) {
    memset(&m_addr, 0, sizeof(address));
    m_addr.sin_family = AF_INET;
    m_addr.sin_port = onBigEndian(port);
    m_addr.sin_addr.s_addr = onBigEndian(address);
}
...
std::ostream& IPv4Address::insert(std::ostream &os ) const {
    uint32_t address = onBigEndian(m_addr.sin_addr.s_addr);
    os << ((address >> 24) & 0xff) << "."
       << ((address >> 16) & 0xff) << "."
       << ((address >> 8 ) & 0xff) << "."
       << ((address      ) & 0xff);
    os << ":" << onBigEndian(m_addr.sin_port);
    return os;
}
...
void IPv4Address::setPort(uint16_t v) {
    m_addr.sin_port = onLittleEndian(v);
}
